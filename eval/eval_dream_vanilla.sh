...
STEPS=32
MAX_NEW_TOKENS=256
TEMPERATURE=1.0  # Lower temperature for vanilla (more deterministic)
DENOISE_TEMP=0.3
TOP_P=0.9
ALG="entropy"
# Penalty parameters
REPETITION_PENALTY=1.0
FILLUP_EOT_PENALTY=1.0  # Basic value for vanilla
DENOISE_EOT_PENALTY=1.0
# Denoising parameters (VANILLA - NO DENOISING)
DENOISE_ALG="none"  # No denoising algorithm for vanilla
DENOISE_MAX_ITER=0  # Set to 0 to disable denoising completely
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo ""
      echo "Options:"
      echo "Dream VANILLA Evaluation Script (BASELINE - NO DENOISING)"
      echo ""
      echo "Model Options:"
...
echo "==== Running Dream-Vanilla evaluation ===="
echo "Generation: steps=$STEPS, max_new_tokens=$MAX_NEW_TOKENS, temperature=$TEMPERATURE"
...
PYTHONPATH=. accelerate launch --main_process_port $PORT -m lm_eval \
  ...
  --seed $SEED \
  --output_path "${OUTPUT_DIR}/${TASKS}_dream_vanilla_steps_${STEPS}_tokens_${MAX_NEW_TOKENS}_temp_${TEMPERATURE}_fillup_eot_${FILLUP_EOT_PENALTY}_seed_${SEED}_BASELINE" \
